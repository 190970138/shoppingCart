...
//		    OrdersService ordersService = ServiceFactory.getOrdersService();
	...
	private void toOrdersList(HttpServletResponse resp, List ordersList)
		...
		for(Iterator<Orders> it = ordersList.iterator();it.hasNext();){
			...
			out.println("					<input type=\"button\" value=\"删除\" onclick=\"javascript:window.location='OrderList.do';\">&nbsp;");
			out.println("					<input type=\"button\" value=\"明细\" onclick=\"javascript:window.location='OrderDetail.do';\">");
			out.println("				</td>");
			out.println("			</tr>");
			i++;
		}
